...
These three properties are what let the committed report be diffed and trusted. If
any of them fails the engine stops being a sensor and becomes a participant.
"""
...
from energy_engine.engine import analyze_document, analyze_folder
from energy_engine.generate import generate_corpus
from energy_engine.report import build_json_report, build_markdown_report
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """A credit file with no documents fails loudly rather than passing vacuously."""
