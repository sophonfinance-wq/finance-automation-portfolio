"""Shared fixtures for the §45L energy-credit engine test suite.
...
from energy_engine.generate import DEFECTS, generate_corpus
from energy_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("credit_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every credit file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one credit file with no planted defect."""
