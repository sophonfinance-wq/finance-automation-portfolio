...
defect is a control nobody has ever seen fire, and a defect that fires nothing is a
control asleep at exactly the moment it was needed.
"""
...
from energy_engine.engine import REGISTRY
from energy_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A credit file carrying a planted defect never rolls up to PASS."""
...
def test_every_finding_carries_a_message(
    ...
    Messages are the product here: a reviewer reading the report should learn *why*
    the control exists, not merely that it failed.
    """
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad gross-credit figure is read by more than one rule -- the gross
    re-derivation control and the grand-total footer -- and both must still report
    rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "rate_gross_credit" in fired
    assert "roll_grand_total" in fired
