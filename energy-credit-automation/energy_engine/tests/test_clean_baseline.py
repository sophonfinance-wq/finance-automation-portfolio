"""The clean credit file must satisfy every control, rule by rule.

Parametrized over the registry rather than asserted as one blanket PASS, so a
regression names the control that broke instead of reporting that "something" in a
26-rule registry is unhappy.
"""
...
from energy_engine.engine import REGISTRY, SEVERITY, analyze_document
from energy_engine.model import DocumentReport, Status, Verdict
...
def test_clean_package_passes_rule(
    ...
    """Each control produces only PASS findings on the clean credit file."""
    fired = [
...
def test_every_rule_reports_something_on_the_clean_package(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean credit file"
    )


def test_clean_package_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole credit file rolls up to PASS."""
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~energy_engine.engine.amount_invalid_finding` and would silently default
    to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same credit file twice yields findings in the same order."""
