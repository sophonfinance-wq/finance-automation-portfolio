"""
Command-line interface for the Energy-Efficient Home Credit engine.
===================================================================
...
::
    ...
    python -m energy_engine <folder> [options]

Analyzes every ``.json`` credit file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m energy_engine",
        description=(
            "Read-only control engine for the IRC §45L energy-efficient home credit: every claimed dwelling unit re-derived per project and fiscal year, gated on its close-of-escrow date and RESNET/HERS certification, at the dated statutory per-unit rate, with roll-forward, net-benefit, partner-allocation and cross-artifact tie-outs. Analyzes seeded, fictional credit files."
        ),
    )
    p.add_argument("folder", help="folder containing .json credit files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional credit file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read credit files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json credit files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} credit file(s))")
