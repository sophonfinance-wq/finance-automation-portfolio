...
from proforma_engine.engine import REGISTRY
from proforma_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A proforma file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad hard cost is read by more than one control -- the hard+soft total
    tie and the net-profit derivation -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "cost_hard_soft_total" in fired
    assert "prof_net_profit_derivation" in fired
