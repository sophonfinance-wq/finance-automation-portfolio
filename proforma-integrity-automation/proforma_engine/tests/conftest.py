"""Shared fixtures for the project-proforma integrity engine test suite.
...
from proforma_engine.engine import analyze_document, analyze_folder
from proforma_engine.generate import DEFECTS, generate_corpus
from proforma_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("proforma_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every proforma file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one proforma file with no planted defect."""
