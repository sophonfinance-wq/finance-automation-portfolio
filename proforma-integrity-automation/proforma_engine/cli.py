"""
Command-line interface for the project-proforma integrity engine.
=================================================================
...
::
    ...
    python -m proforma_engine <folder> [options]

Analyzes every ``.json`` proforma file in ``<folder>`` (read-only) and prints
the exceptions. Optionally writes the full markdown and JSON reports.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m proforma_engine",
        description=(
            "Read-only control engine for a real-estate developer's quarterly project-proforma reporting: every active project's proforma re-derived from its base ledger and held to the tie-out registry -- source-and-use balance, cost-budget equalities, interest-reserve adequacy, the profit/waterfall/distribution chain, margin and per-NSF re-derivation, current-vs-prior variance, and the reporting-calendar completeness and due-date gate. Analyzes seeded, fictional proforma files."
        ),
    )
    p.add_argument("folder", help="folder containing .json proforma files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional proforma file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read proforma files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json proforma files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} proforma file(s))")
