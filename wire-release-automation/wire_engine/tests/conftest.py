"""Shared fixtures for the wire & transfer release-control engine test suite.
...
from wire_engine.engine import analyze_document, analyze_folder
from wire_engine.generate import DEFECTS, generate_corpus
from wire_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("release_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every release-control file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one release-control file with no planted defect."""
