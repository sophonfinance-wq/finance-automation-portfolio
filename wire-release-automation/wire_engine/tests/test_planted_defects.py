...
from wire_engine.engine import REGISTRY
from wire_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A release-control file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad amount is read by more than one rule -- the funding-sufficiency
    control and the releasable recompute -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "fund_sufficient_balance" in fired
    assert "rpt_release_flag_recomputes" in fired
