"""
Command-line interface for the wire & transfer release-control engine.
======================================================================
...
::
    ...
    python -m wire_engine <folder> [options]

Analyzes every ``.json`` release-control file in ``<folder>`` (read-only) and
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m wire_engine",
        description=(
            "Read-only control engine for an operating entity's outbound wire and "
            "transfer release: every packet proved segregated, dual-authorized, sent to "
            "an approved beneficiary over a valid routing number, funded, free of a "
            "duplicate, and moved through the workflow in order. Analyzes seeded, "
            "fictional release-control files."
        ),
    )
    p.add_argument("folder", help="folder containing .json release-control files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional release-control file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read release-control files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json release-control files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} release-control file(s))")
