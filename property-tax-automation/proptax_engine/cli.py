"""
Command-line interface for the Property tax engine.
==============================================================
...
::
    ...
    python -m proptax_engine <folder> [options]

Analyzes every ``.json`` roll file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Analyzes seeded, fictional roll files."
        ),
    )
    p.add_argument("folder", help="folder containing .json roll files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional roll file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read roll files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json roll files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} roll file(s))")
