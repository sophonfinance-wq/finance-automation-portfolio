...
from proptax_engine.engine import REGISTRY
from proptax_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A roll file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    readable. A single bad accrual amount is read by more than one rule -- the
    parcel's own footing to its annual charge and the portfolio-wide tie to the
    ledger -- and both must still report rather than aborting at the first bad
    value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "acr_periods_tie_annual" in fired
    assert "acr_ties_gl_liability" in fired
