...
from proptax_engine.engine import analyze_document, analyze_folder
from proptax_engine.generate import DEFECTS, generate_corpus
from proptax_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("roll_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every roll file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one roll file with no planted defect."""
