"""
Money primitives for the property tax engine.
=============================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. An installment
schedule that tolerates a penny against the annual tax it splits has not been
reconciled; it has been rounded until it looked reconciled, which is the opposite
thing. The same applies to a closing proration: the seller's share and the
buyer's share must sum to the tax, exactly, or somebody paid twice.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"parcel[0284-117-03]/annual_tax_cents"``.
    value:
