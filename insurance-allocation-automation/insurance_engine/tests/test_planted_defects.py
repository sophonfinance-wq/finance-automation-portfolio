...
from insurance_engine.engine import REGISTRY
from insurance_engine.generate import DEFECTS
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad allocation amount is read by more than one rule -- the policy's
    own footing to its premium and the portfolio-wide tie to job cost -- and both
    must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "alloc_sums_to_premium" in fired
    assert "gl_allocation_ties_job_cost" in fired
