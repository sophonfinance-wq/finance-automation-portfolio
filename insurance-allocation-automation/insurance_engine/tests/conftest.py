"""Shared fixtures for the insurance cost allocation engine test suite.
...
from insurance_engine.engine import analyze_document, analyze_folder
from insurance_engine.generate import DEFECTS, generate_corpus
from insurance_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("programme_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every programme file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one programme file with no planted defect."""
