"""
Command-line interface for the Insurance cost allocation engine.
==============================================================
...
::
    ...
    python -m insurance_engine <folder> [options]

Analyzes every ``.json`` programme file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m insurance_engine",
        description=(
            "Read-only control engine for a developer's insurance programme cost allocation: master premium apportioned across projects, builder's risk, and carrier audit true-up. Analyzes seeded, fictional programme files."
        ),
    )
    p.add_argument("folder", help="folder containing .json programme files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional programme file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read programme files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json programme files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} programme file(s))")
