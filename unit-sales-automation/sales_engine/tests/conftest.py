"""Shared fixtures for the unit sales tie-out engine test suite.
...
from sales_engine.engine import analyze_document, analyze_folder
from sales_engine.generate import DEFECTS, generate_corpus
from sales_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("sales_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every sales file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one sales file with no planted defect."""
