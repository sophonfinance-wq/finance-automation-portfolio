...
from sales_engine.engine import REGISTRY
from sales_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A sales file carrying a planted defect never rolls up to PASS."""
...
def test_flag_only_defects_roll_up_to_review(
    ...
    """
    for name in ("discount_over_band", "proof_not_received"):
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad net-proceeds cell is read by more than one rule -- the
    re-derivation control on the Matrix row and the revenue rollup that sums it --
    and both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "price_net_proceeds_recomputes" in fired
    assert "rpt_revenue_ties" in fired
