"""
Command-line interface for the Unit sales and commission tie-out engine.
========================================================================
...
::
    ...
    python -m sales_engine <folder> [options]

Analyzes every ``.json`` sales file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m sales_engine",
        description=(
            "Read-only control engine for a residential for-sale project's unit sales Matrix: every unit's revenue per square foot, net proceeds and total commission re-derived from the prices beside them, every commission split tied to the total it divides, and every sold unit reconciled to the Sold/Close tab. Analyzes seeded, fictional sales files."
        ),
    )
    p.add_argument("folder", help="folder containing .json sales files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional sales file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read sales files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json sales files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} sales file(s))")
