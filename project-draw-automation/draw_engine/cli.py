"""
Command-line interface for the project draw engine.
===================================================
...
::
    ...
    python -m draw_engine <folder> [options]

Analyzes every ``.json`` draw package in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m draw_engine",
        description=(
            "Read-only control engine for construction loan draw requests. "
            "Analyzes seeded, fictional draw packages."
        ),
    )
    p.add_argument("folder", help="folder containing .json draw packages")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional draw package(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read draw packages in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json draw packages found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} draw package(s))")
