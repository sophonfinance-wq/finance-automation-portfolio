...
from draw_engine.engine import REGISTRY
from draw_engine.generate import DEFECTS
from draw_engine.model import DocumentReport, Status, Verdict
...
def test_defect_package_is_not_clean(
    ...
    """A package carrying a planted defect never rolls up to PASS."""
...
def test_defect_declares_itself(
    ...
    """Each defect package is labelled, so a reader can tell what it demonstrates."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    """One malformed amount does not stop the rest of the form being examined.

    The guard is per-row precisely so the report still covers the lines that were
    readable. If a single bad value aborted the check, the reader would see no
    exception for lines that were never looked at.
    """
    report = by_defect["amount_not_integer"]
    # form_columns_foot still reaches its other columns and reports on them.
    fired = report.rules_fired()
    assert "form_columns_foot" in fired
