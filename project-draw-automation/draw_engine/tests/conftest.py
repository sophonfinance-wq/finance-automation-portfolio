...
from draw_engine.engine import analyze_document, analyze_folder
from draw_engine.generate import DEFECTS, generate_corpus
from draw_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("draw_corpus")
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one package with no planted defect."""
