"""The clean package must satisfy every control, rule by rule.
...
from draw_engine.engine import REGISTRY, SEVERITY, analyze_document
...
def test_clean_package_passes_rule(
    ...
    """Each control produces only PASS findings on the clean package."""
    fired = [
...
def test_every_rule_reports_something_on_the_clean_package(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean package"
    )
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~draw_engine.engine.amount_invalid_finding` and would silently default
    to FAIL.
    """
