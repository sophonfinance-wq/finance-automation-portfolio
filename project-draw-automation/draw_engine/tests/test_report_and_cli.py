...
from draw_engine.cli import main
from draw_engine.engine import REGISTRY, analyze_folder
from draw_engine.model import DocumentReport, Status, Verdict
from draw_engine.report import (
    build_json_report,
...
def test_json_report_shape(reports: list[DocumentReport]) -> None:
    ...
    assert doc["package_count"] == len(reports)
    assert doc["rule_count"] == len(REGISTRY)
    assert len(doc["rules"]) == len(REGISTRY)
    assert len(doc["packages"]) == len(reports)
