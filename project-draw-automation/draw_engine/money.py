"""
Money primitives for the project draw engine.
=============================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A draw
reconciliation that tolerates a penny has not reconciled.
...
Two more exist because a draw specifically needs them:

* :func:`percent_complete_bps` derives progress as integer basis points, which
  is how the contingency ceiling is expressed.
* :func:`apply_rate` applies an integer basis-point rate with truncating
  division, matching how a billing platform derives a retention line -- so the
...
class AmountInvalidError(ValueError):
    ...
    field:
        Dotted path of the offending field, e.g. ``"DRAW-2027-0004/request"``.
    value:
...
def percent_complete_bps(actual_cents: int, budget_cents: int) -> int:
    ...
    Floor division is deliberate and conservative in the direction that matters:
    the result is the **ceiling** on how much contingency may be drawn, so
    rounding it down can only ever hold a draw to a stricter limit than the
    procedure allows. Rounding up could licence a dollar of contingency the work
    has not yet earned.

    A zero or negative budget returns ``0``: no budget means no earned progress,
    and therefore no contingency entitlement. A negative actual -- a net credit
    against the class -- also returns ``0`` rather than a negative ceiling.

    Args:
        actual_cents: Costs incurred to date against the class.
        budget_cents: Budget for the class.
