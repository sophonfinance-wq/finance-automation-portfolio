"""
Money primitives for the gross-receipts & excise tax engine.
============================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A taxable base
that tolerates a penny against the general-ledger revenue pull is not the
ledger's number, and the tax struck on it is a figure nobody can trace. Rates
are applied as integer basis points with truncating division, so the tax the
worksheet filed and the tax its own receipts and rate produce can be compared
exactly -- which is the whole claim this engine makes.
