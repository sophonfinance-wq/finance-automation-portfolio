...
from grt_engine.engine import REGISTRY
from grt_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A reconciliation file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad taxable base is read by more than one rule -- the pre-credit tax
    re-derivation and the base re-derivation -- and both must still report rather
    than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "rate_tax_before_credit_recomputes" in fired
    assert "ded_base_recomputes" in fired
