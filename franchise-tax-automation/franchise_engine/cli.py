"""
Command-line interface for the combined-group franchise-tax engine.
===================================================================
...
::
    ...
    python -m franchise_engine <folder> [options]

Analyzes every ``.json`` combined-return file in ``<folder>`` (read-only) and
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m franchise_engine",
        description=(
            "Read-only control engine for a combined-group franchise (margin) tax return: the affiliate roster rebuilt from the prior year, group revenue tied to the consolidated trial balance, the single receipts apportionment factor recomputed, and the margin tax re-derived. Analyzes seeded, fictional combined-return files."
        ),
    )
    p.add_argument("folder", help="folder containing .json combined-return files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional combined-return file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read combined-return files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json combined-return files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} combined-return file(s))")
