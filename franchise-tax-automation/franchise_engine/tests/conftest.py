"""Shared fixtures for the combined-group franchise-tax engine test suite.
...
from franchise_engine.engine import analyze_document, analyze_folder
from franchise_engine.generate import DEFECTS, generate_corpus
from franchise_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("franchise_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every combined-return file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one combined-return file with no planted defect."""
