...
from franchise_engine.engine import REGISTRY
from franchise_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A combined-return file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad group-revenue figure is read by more than one rule -- the
    revenue tie and the margin re-derivation -- and both must still report rather
    than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "mgn_revenue_ties_tb" in fired
    assert "mgn_margin_recomputes" in fired
