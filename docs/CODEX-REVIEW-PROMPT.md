...
> **`pytest -m "not site_tooling"` → 463,860 collected (curated suite) · `pytest -m site_tooling` → 54 collected (separate public-datasheet guards) · `SWEEP=1 pytest -m "not site_tooling"` → 1,648,990 (property sweep) · 47 systems** (verified 2026-07-24)
...
463,860-test curated engine suite.
