"""Shared fixtures for the financing-execution engine test suite.
...
from financing_engine.engine import analyze_document, analyze_folder
from financing_engine.generate import DEFECTS, generate_corpus
from financing_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("financing_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every financing report in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one financing report with no planted defect."""
