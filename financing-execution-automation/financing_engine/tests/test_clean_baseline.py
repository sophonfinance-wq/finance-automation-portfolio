"""The clean financing report must satisfy every control, rule by rule.
...
from financing_engine.engine import REGISTRY, SEVERITY, analyze_document
from financing_engine.model import DocumentReport, Status, Verdict

RULE_IDS = [rule_id for rule_id, _fn in REGISTRY]


@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_clean_report_passes_rule(
    rule_id: str, clean_report: DocumentReport
) -> None:
    """Each control produces only PASS findings on the clean financing report."""
    fired = [
...
@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_every_rule_reports_something_on_the_clean_report(
    rule_id: str, clean_report: DocumentReport
) -> None:
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean financing report"
    )


def test_clean_report_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole financing report rolls up to PASS."""
    assert clean_report.verdict is Verdict.PASS


def test_clean_report_has_no_flags(clean_report: DocumentReport) -> None:
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~financing_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same financing report twice yields findings in the same order."""
