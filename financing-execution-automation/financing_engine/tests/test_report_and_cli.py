...
from financing_engine.cli import main
from financing_engine.engine import REGISTRY, analyze_folder
from financing_engine.model import DocumentReport, Status, Verdict
from financing_engine.report import (
    build_json_report,
...
def test_markdown_report_has_structure(reports: list[DocumentReport]) -> None:
    ...
    assert "## Financing reports" in md
    assert md.endswith("\n")
...
def test_markdown_lists_every_report(reports: list[DocumentReport]) -> None:
    md = build_markdown_report(reports)
    for report in reports:
