...
from financing_engine.engine import REGISTRY
from financing_engine.generate import DEFECTS
...
@pytest.mark.parametrize("name", defect_names())
def test_defect_report_is_not_clean(
    name: str, by_defect: dict[str, DocumentReport]
) -> None:
    """A financing report carrying a planted defect never rolls up to PASS."""
...
def test_every_finding_carries_a_message(
    ...
    Messages are the product here: a reviewer reading the report should learn
...
def test_amount_invalid_is_contained_to_the_run(
    by_defect: dict[str, DocumentReport]
) -> None:
    """One malformed amount does not stop the rest of the file being read.

    The financing total control raises on the bad amount, but the whole registry
    still runs to completion -- the structural precondition still passes and the
    other controls still report -- rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    assert "rpt_financing_total_ties" in report.rules_fired()
    # The rest of the registry still executed: many PASS findings remain.
    passes = [f for f in report.findings if f.status is Status.PASS]
