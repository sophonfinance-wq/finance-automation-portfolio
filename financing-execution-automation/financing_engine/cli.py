"""
Command-line interface for the Financing Execution & Schedule-Variance engine.
==============================================================================
...
::
    ...
    python -m financing_engine <folder> [options]

Analyzes every ``.json`` financing report in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m financing_engine",
        description=(
            "Read-only control engine for a developer's monthly 'Upcoming Financings' report: every milestone variance re-derived, this month's Prior tied to last month's Current, the Original baseline frozen, the milestone playbook complete per financing type, and overdue milestones date-gated. Analyzes seeded, fictional financing reports."
        ),
    )
    p.add_argument("folder", help="folder containing .json financing reports")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional financing report(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read financing reports in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json financing reports found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} financing report(s))")
