"""Shared fixtures for the baseline & version drift engine test suite.
...
from baseline_engine.generate import DEFECTS, generate_corpus
from baseline_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("baseline_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every cycle file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one cycle file with no planted defect."""
