...
from baseline_engine.engine import analyze_document, analyze_folder
from baseline_engine.generate import generate_corpus
from baseline_engine.report import build_json_report, build_markdown_report
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """A cycle file with no documents fails loudly rather than passing vacuously."""
...
def test_no_control_reads_the_system_clock(corpus: Path) -> None:
    """Findings depend only on the file's own reporting period.

    A control that reached for today's date would make the corpus pass or fail
    according to when CI happened to run. The reporting period is fictional and in
