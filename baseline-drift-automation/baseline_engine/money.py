"""
Money primitives for the baseline & version drift engine.
=========================================================

Every monetary amount inside the engine is an **integer number of cents**, so
arithmetic is exact and cross-version comparisons are deterministic.
Floating-point dollars are never used for accumulation or comparison; they
appear only at the formatting edge. Dataclass fields and JSON keys carry a
``_cents`` suffix.

Comparisons are exact ``==`` -- there is **no tolerance band**. This engine
exists because copies of one budget drift apart, and a tolerance band is the
mechanism by which drift becomes invisible: a line that is "close enough" in
three versions is a line nobody reconciles until the three disagree by real
money. A materiality threshold does appear in this engine, but only to grade a
*confirmed* difference for review -- never to decide whether a difference exists.
...
* :func:`split_evenly` distributes an amount across N parts so the parts sum back
  to the whole (the rounding remainder lands on the final part). A derived
  schedule amortised in equal instalments is built with it.
* :func:`allocate_by_ratio` splits an amount by integer basis-point weights using
  the largest-remainder method, guaranteeing the parts sum to the whole. Member
  funding splits are derived with it.
...
* :func:`percent_complete_bps` derives a ratio as integer basis points, floored,
  used wherever a consumed-against-budget proportion is reported rather than
  acted on.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"workpaper[E-01/1206]/value_cents"``.
    value:
...
def percent_complete_bps(actual_cents: int, budget_cents: int) -> int:
    ...
    Returns:
        Percent consumed in basis points (``2500`` == 25.00%).
    """
