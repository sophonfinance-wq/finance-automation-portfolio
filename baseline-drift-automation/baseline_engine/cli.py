"""
Command-line interface for the baseline & version drift engine.
===============================================================
...
::
    ...
    python -m baseline_engine <folder> [options]

Analyzes every ``.json`` cycle file in ``<folder>`` (read-only) and prints the
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional cycle file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read cycle files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json cycle files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} cycle file(s))")
