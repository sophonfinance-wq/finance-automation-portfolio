"""Shared fixtures for the earnest-money deposit trust engine test suite.
...
from deposit_engine.engine import analyze_document, analyze_folder
from deposit_engine.generate import DEFECTS, generate_corpus
from deposit_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("deposit_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every deposit file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one deposit file with no planted defect."""
