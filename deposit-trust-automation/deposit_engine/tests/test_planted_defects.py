...
from deposit_engine.engine import REGISTRY
from deposit_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A deposit file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad agent figure is read by more than one rule -- the per-unit
    tie-out and the summary recompute -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "tie_agent_variance_zero" in fired
    assert "rpt_unit_variance_recomputes" in fired
