"""
Command-line interface for the earnest-money deposit trust engine.
==================================================================
...
::
    ...
    python -m deposit_engine <folder> [options]

Analyzes every ``.json`` deposit file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m deposit_engine",
        description=(
            "Read-only control engine for a residential developer's pre-close earnest-money deposit trust: the internal deposit ledger, the escrow agent's record and the construction-loan paydown register tied three ways per unit to zero variance, with cancellation/forfeiture split gates and segregation identities. Analyzes seeded, fictional deposit files."
        ),
    )
    p.add_argument("folder", help="folder containing .json deposit files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional deposit file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read deposit files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json deposit files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} deposit file(s))")
