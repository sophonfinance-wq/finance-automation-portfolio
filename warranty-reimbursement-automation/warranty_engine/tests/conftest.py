"""Shared fixtures for the draw engine test suite.
...
from warranty_engine.engine import analyze_document, analyze_folder
from warranty_engine.generate import DEFECTS, generate_corpus
from warranty_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("warranty_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every package in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one claim file with no planted defect."""
