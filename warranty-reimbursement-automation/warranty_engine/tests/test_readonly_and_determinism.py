...
from warranty_engine.generate import generate_corpus
from warranty_engine.report import build_json_report, build_markdown_report
...
def test_folder_analysis_is_sorted(corpus: Path) -> None:
    """Packages are analyzed in sorted order regardless of filesystem order."""
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """A claim file with no documents fails loudly rather than passing vacuously."""
