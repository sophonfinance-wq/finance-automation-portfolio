...
from warranty_engine.money import (
    AMOUNT_INVALID,
    BPS_FULL,
    AmountInvalidError,
    allocate_by_ratio,
...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize(
    "amount,periods", [(100, 3), (1, 3), (0, 5), (-100, 3), (999_999, 7), (10, 1)]
)
def test_split_evenly_sums_to_whole(amount: int, periods: int) -> None:
    parts = split_evenly(amount, periods)
...
def test_apply_rate_truncates() -> None:
    """Truncation is the contract, so a derived retention line compares with ==."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0, 100, 0),
        (100, 100, BPS_FULL),
        (200, 100, 20000),                    # over budget reports over 100%
    ],
)
...
def test_percent_complete_bps_zero_or_negative_budget(budget: int) -> None:
    """No budget means no earned progress, and therefore no contingency ceiling."""
...
def test_percent_complete_bps_non_positive_actual(actual: int) -> None:
    """A net credit against a class does not create a negative ceiling."""
    assert percent_complete_bps(actual, 1_000) == 0


def test_percent_complete_floors_rather_than_rounds() -> None:
    """Flooring can only ever hold contingency to a stricter limit.

    Rounding up would licence a dollar of contingency the work has not earned,
    so the direction of the rounding is a control decision, not a detail.
    """
