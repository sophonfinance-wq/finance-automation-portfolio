...
from intercompany_engine.engine import REGISTRY
from intercompany_engine.generate import DEFECTS
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    readable. A single bad transaction amount is read by more than one rule -- the
    ledger footing and the roll-forward of the pair it moves -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "led_totals_foot" in fired
    assert "led_rollforward_ties" in fired
