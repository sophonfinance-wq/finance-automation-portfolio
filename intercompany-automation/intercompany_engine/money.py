"""
Money primitives for the intercompany reciprocity engine.
=========================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A due-from balance
that tolerates a penny against the due-to it faces has not been reconciled; it has
been rounded until it looked reconciled, which is the opposite thing.
...
class AmountInvalidError(ValueError):
    ...
    field:
        Dotted path of the offending field, e.g. ``"due_from[HOLD->PRED]/closing"``.
    value:
