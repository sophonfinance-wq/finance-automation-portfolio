"""
Command-line interface for the equity-waterfall engine.
=======================================================
...
::
    ...
    python -m waterfall_engine <folder> [options]

Analyzes every ``.json`` deal file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m waterfall_engine",
        description=(
            "Read-only control engine for a real-estate JV distribution waterfall: "
            "the preferred-return accrual, capital-account roll-forward, tier "
            "sequencing, hurdle boundary, promote split, dilution and realized "
            "returns all re-derived from the executed agreement's mechanics and the "
            "contribution history. Analyzes seeded, fictional deal files."
        ),
    )
    p.add_argument("folder", help="folder containing .json deal files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional deal file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read deal files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json deal files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} deal file(s))")
