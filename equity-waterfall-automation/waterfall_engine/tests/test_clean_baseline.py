"""The clean deal file must satisfy every control, rule by rule.
...
from waterfall_engine.engine import REGISTRY, SEVERITY, analyze_document
from waterfall_engine.model import DocumentReport, Status, Verdict

RULE_IDS = [rule_id for rule_id, _fn in REGISTRY]


@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_clean_deal_passes_rule(rule_id: str, clean_report: DocumentReport) -> None:
    """Each control produces only PASS findings on the clean deal file."""
    fired = [
...
@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_every_rule_reports_something_on_the_clean_deal(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean deal file"
    )


def test_clean_deal_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole deal file rolls up to PASS."""
    assert clean_report.verdict is Verdict.PASS


def test_clean_deal_has_no_flags(clean_report: DocumentReport) -> None:
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~waterfall_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same deal file twice yields findings in the same order."""
