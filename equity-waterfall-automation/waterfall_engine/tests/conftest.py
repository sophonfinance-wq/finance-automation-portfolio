"""Shared fixtures for the equity-waterfall engine test suite.
...
from waterfall_engine.engine import analyze_document, analyze_folder
from waterfall_engine.generate import DEFECTS, generate_corpus
from waterfall_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("waterfall_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every deal file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one deal file with no planted defect."""
