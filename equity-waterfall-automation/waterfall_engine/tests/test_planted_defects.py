...
from waterfall_engine.engine import REGISTRY
from waterfall_engine.generate import DEFECTS
...
@pytest.mark.parametrize("name", defect_names())
def test_defect_deal_is_not_clean(
    name: str, by_defect: dict[str, DocumentReport]
) -> None:
    """A deal file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    The total distributed is read by more than one control -- the leakage tie-out
    and the within-available gate -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "dist_no_leakage" in fired
    assert "dist_within_available" in fired
