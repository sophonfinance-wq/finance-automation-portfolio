"""Shared fixtures for the entity good-standing engine test suite.
...
from standing_engine.engine import analyze_document, analyze_folder
from standing_engine.generate import DEFECTS, generate_corpus
from standing_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("standing_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every standing file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one standing file with no planted defect."""
