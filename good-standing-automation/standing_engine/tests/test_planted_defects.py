...
from standing_engine.engine import REGISTRY
from standing_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A standing file carrying a planted defect never rolls up to PASS."""
...
def test_every_finding_carries_a_message(
    ...
    Messages are the product here: a manager reading the report should learn *why*
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad franchise-tax amount is read by more than one rule -- the amount
    adequacy control and the entity-standing recompute -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "tax_amount_meets" in fired
    assert "roll_standing_recomputes" in fired
