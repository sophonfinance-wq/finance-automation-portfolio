"""
Command-line interface for the entity good-standing engine.
===========================================================
...
::
    ...
    python -m standing_engine <folder> [options]

Analyzes every ``.json`` standing file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m standing_engine",
        description=(
            "Read-only control engine for a real-estate family's entity good-standing: every entity proved, state by state, to have its annual/biennial report filed on time, its franchise tax paid in the statutory amount, its registered agent current and its Secretary-of-State status ACTIVE. Analyzes seeded, fictional standing files."
        ),
    )
    p.add_argument("folder", help="folder containing .json standing files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional standing file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read standing files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json standing files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} standing file(s))")
