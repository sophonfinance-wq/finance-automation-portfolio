...
from capitalize_engine.engine import REGISTRY
from capitalize_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A capitalization file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad expenditure is read by more than one rule -- the non-negativity
    control and the capitalized re-derivation -- and both must still report rather
    than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "data_ape_nonneg" in fired
    assert "cap_rederives" in fired
