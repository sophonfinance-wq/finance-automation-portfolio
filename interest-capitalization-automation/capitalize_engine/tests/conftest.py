"""Shared fixtures for the interest-capitalization engine test suite.
...
from capitalize_engine.engine import analyze_document, analyze_folder
from capitalize_engine.generate import DEFECTS, generate_corpus
from capitalize_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("capitalization_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every capitalization file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one capitalization file with no planted defect."""
