"""
Command-line interface for the Section 263A interest-capitalization engine.
===========================================================================
...
::
    ...
    python -m capitalize_engine <folder> [options]

Analyzes every ``.json`` capitalization file in ``<folder>`` (read-only) and
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m capitalize_engine",
        description=(
            "Read-only control engine for a real-estate developer's Section 263A "
            "avoided-cost interest-capitalization schedule: every active project proved, "
            "quarter by quarter, to capitalize the interest the method requires, foot to "
            "its annual totals, and tie the year-over-year comparison. Analyzes seeded, "
            "fictional capitalization files."
        ),
    )
    p.add_argument("folder", help="folder containing .json capitalization files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional capitalization file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read capitalization files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json capitalization files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} capitalization file(s))")
