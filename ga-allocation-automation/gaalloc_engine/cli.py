"""
Command-line interface for the G&A allocation engine.
=====================================================
...
::
    ...
    python -m gaalloc_engine <folder> [options]

Analyzes every ``.json`` allocation file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m gaalloc_engine",
        description=(
            "Read-only control engine for a holding entity's recurring G&A expense allocation: shared overhead, corporate G&A and metered postage pushed out to operating entities and projects on a driver, with every share, amount and journal entry re-derived and compared. Analyzes seeded, fictional allocation files."
        ),
    )
    p.add_argument("folder", help="folder containing .json allocation files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional allocation file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read allocation files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json allocation files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} allocation file(s))")
