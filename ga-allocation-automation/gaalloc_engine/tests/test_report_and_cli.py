...
from gaalloc_engine.cli import main
from gaalloc_engine.engine import REGISTRY, analyze_folder
from gaalloc_engine.model import DocumentReport, Status, Verdict
from gaalloc_engine.report import (
    build_json_report,
...
def test_every_control_fires_somewhere_in_the_corpus(
    reports: list[DocumentReport],
) -> None:
    """No rule ships with a zero exception count across the whole corpus.

    A control that never fires anywhere has not been demonstrated, and the report
    is the artifact that has to show it.
    """
    doc = build_json_report(reports)
    silent = [r["rule"] for r in doc["rules"] if r["exceptions"] == 0]
    assert not silent, f"controls that never fired anywhere: {silent}"
...
def test_markdown_states_the_engine_never_posts(
    reports: list[DocumentReport],
) -> None:
    """The engine re-derives a journal entry; the artifact says it does not post it."""
    md = build_markdown_report(reports).lower()
