...
which, for an engine whose subject is a journal entry, is the one thing it must
never be.
"""
...
from gaalloc_engine.engine import analyze_document, analyze_folder
from gaalloc_engine.generate import generate_corpus
from gaalloc_engine.report import build_json_report, build_markdown_report
...
def test_no_control_reads_the_system_clock(corpus: Path) -> None:
    """Every accounting-date test is made against the period stamped in the file.

    A control that reached for today's date would produce a different report
    tomorrow, which is the same failure as a timestamp in the output.
    """
    reports = analyze_folder(corpus)
    first = [r.to_dict() for r in reports]
    again = [r.to_dict() for r in analyze_folder(corpus)]
    assert first == again
    md = build_markdown_report(reports)
    assert "2031-" in md or "allocation" in md
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """An allocation file with no documents fails loudly rather than passing vacuously."""
