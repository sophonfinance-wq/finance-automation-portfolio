"""Shared fixtures for the G&A allocation engine test suite.
...
from gaalloc_engine.generate import DEFECTS, generate_corpus
from gaalloc_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("allocation_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every allocation file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one allocation file with no planted defect."""
