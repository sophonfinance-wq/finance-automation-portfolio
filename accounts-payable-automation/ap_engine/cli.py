"""
Command-line interface for the accounts payable engine.
=======================================================
...
::
    ...
    python -m ap_engine <folder> [options]

Analyzes every ``.json`` posting document set in ``<folder>`` (read-only) and
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m ap_engine",
        description=(
            "Read-only control engine for the accounts payable cycle. "
            "Analyzes seeded, fictional posting document sets."
        ),
    )
    p.add_argument("folder", help="folder containing .json posting document sets")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional document set(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read document sets in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json document sets found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} document set(s))")
