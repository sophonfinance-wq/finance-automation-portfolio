"""
Command-line interface for the project variance engine.
======================================================
...
::
    ...
    python -m variance_engine <folder> [options]

Analyzes every ``.json`` project report in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m variance_engine",
        description=(
            "Read-only control engine for the periodic project report a developer re-issues off every development proforma: the current period restated against the prior period and the approved Business Plan baseline, each cost line broken into Total Budget / Cost to Date / Cost to Complete, and each milestone reported against a frozen baseline schedule. Analyzes seeded, fictional project reports."
        ),
    )
    p.add_argument("folder", help="folder containing .json project reports")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional project report(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read project reports in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json project reports found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} project report(s))")
