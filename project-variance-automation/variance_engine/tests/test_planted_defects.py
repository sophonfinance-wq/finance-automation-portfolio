...
from variance_engine.engine import REGISTRY
from variance_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A project report carrying a planted defect never rolls up to PASS."""
...
def test_flag_only_defects_roll_up_to_review(
    ...
    """
    for name in ("milestone_slipped", "watchlist_overstated"):
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad cost figure is read by more than one rule -- the Cost to Complete
    remainder and the expense tie-out to the budget above it -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "bud_cost_to_complete_foots" in fired
    assert "eco_expense_foots_budget" in fired
