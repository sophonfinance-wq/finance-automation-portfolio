"""Shared fixtures for the project variance engine test suite.
...
from variance_engine.engine import analyze_document, analyze_folder
from variance_engine.generate import DEFECTS, generate_corpus
from variance_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("variance_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every project report in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one project report with no planted defect."""
