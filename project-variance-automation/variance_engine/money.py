"""
Money primitives for the project variance engine.
=================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A variance column
that tolerates a penny is not the difference of the two columns it sits between,
and a Cost to Complete that tolerates a penny is not the remainder of its budget.
A restatement engine lives or dies on that exactness: the whole product is the
claim that a stated figure re-derives from the base facts, and a tolerance band
would quietly retire the claim.
