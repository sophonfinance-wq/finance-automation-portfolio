"""Shared fixtures for the insurance compliance engine test suite.
...
from coi_engine.engine import analyze_document, analyze_folder
from coi_engine.generate import DEFECTS, generate_corpus
from coi_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("compliance_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every compliance file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one compliance file with no planted defect."""
