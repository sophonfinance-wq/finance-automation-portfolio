...
from coi_engine.engine import REGISTRY
from coi_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A compliance file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad limit is read by more than one rule -- the per-occurrence
    adequacy control and the party-compliant recompute -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "cov_per_occurrence_meets" in fired
    assert "rpt_party_compliant_recomputes" in fired
