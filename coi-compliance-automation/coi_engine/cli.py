"""
Command-line interface for the Insurance compliance engine.
===========================================================
...
::
    ...
    python -m coi_engine <folder> [options]

Analyzes every ``.json`` compliance file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m coi_engine",
        description=(
            "Read-only control engine for a residential developer's certificate-of-insurance compliance: every consultant, general contractor and subcontractor proved on file, unexpired, and meeting the contract-required limits and endorsements. Analyzes seeded, fictional compliance files."
        ),
    )
    p.add_argument("folder", help="folder containing .json compliance files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional compliance file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read compliance files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json compliance files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} compliance file(s))")
