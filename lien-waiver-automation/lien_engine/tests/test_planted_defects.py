...
from lien_engine.engine import REGISTRY
from lien_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A portfolio file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad payment amount is read by more than one rule -- the payment
    register's own positivity check and the waiver-to-payment amount tie -- and
    both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "pay_amount_positive" in fired
    assert "wvr_amount_ties_payment" in fired
