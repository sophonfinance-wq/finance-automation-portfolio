"""Shared fixtures for the lien waiver tracking engine test suite.
...
from lien_engine.engine import analyze_document, analyze_folder
from lien_engine.generate import DEFECTS, generate_corpus
from lien_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("portfolio_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every portfolio file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one portfolio file with no planted defect."""
