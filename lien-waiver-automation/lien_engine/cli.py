"""
Command-line interface for the lien waiver tracking engine.
===========================================================
...
::
    ...
    python -m lien_engine <folder> [options]

Analyzes every ``.json`` portfolio file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m lien_engine",
        description=(
            "Read-only control engine for a developer's mechanics-lien waiver "
            "coverage: every progress payment matched to a waiver of the right kind, "
            "through the right date, at every tier. Analyzes seeded, fictional "
            "portfolio files."
        ),
    )
    p.add_argument("folder", help="folder containing .json portfolio files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional portfolio file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read portfolio files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json portfolio files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} portfolio file(s))")
