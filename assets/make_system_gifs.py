...
SYSTEMS = [
    ...
    },
    {
        ...
        "subtitle": "record - transcribe - ingest - cite, with full provenance",
        "code_caption": "deterministic retrieval + citation governance",
        "tests": 50, "footer": "stdlib TF-IDF - no embeddings / LLM / network - fictional transcripts",
        "pill": "CITED", "badge_color": GREEN, "badge": "ANSWER CITED",
        "badge_sub": "verbatim quote + date - time - speaker, or it refuses",
        "kpis": [("4", "Meetings"), ("15", "Cards"), ("0", "Uncited")],
        "code": [
            [kw("def"), (" cite", C_FN), ("(self, query):", C_PLN)],
            [("    # one paste-ready, sourced footnote — or nothing", C_COM)],
            [("    hits = self.search(query)", C_PLN)],
            [("    hits = [h ", C_PLN), kw("for"), (" h ", C_PLN), kw("in"), (" hits ", C_PLN), kw("if"), (" h.score >= MIN_RELEVANCE]", C_PLN)],
            [("    ", C_PLN), kw("if"), (" ", C_PLN), kw("not"), (" hits:", C_PLN)],
            [("        ", C_PLN), kw("return"), (" REFUSE   ", C_PLN), ("# the brain does not guess", C_COM)],
            [("    card = best_authority(hits)  ", C_PLN), ("# decision / rule / definition", C_COM)],
            [("    ", C_PLN), kw("return"), (" citation_block(card)  ", C_PLN), ("# verbatim + provenance", C_COM)],
        ],
        "console": [
            [("$ ", (79, 70, 229)), ('python -m brain_engine --cite "ROC beyond basis"', TEXT)],
            [("> Decision: any return of capital in excess of the", DIM)],
            [("  investor's remaining basis is a deemed gain.", DIM)],
            [("  [Return of Capital Review - 2025-02-03 - 00:03:41 - L. Frost]", FAINT)],
            [("$ ", (79, 70, 229)), ('python -m brain_engine ask "office lunch policy"', TEXT)],
            [("> No sourced answer - the brain does not guess.", AMBER, True)],
        ],
    },
]
