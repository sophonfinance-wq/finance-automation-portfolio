"""The clean calendar file must satisfy every control, rule by rule.
...
from filing_engine.engine import REGISTRY, SEVERITY, analyze_document
...
def test_clean_file_passes_rule(rule_id: str, clean_report: DocumentReport) -> None:
    """Each control produces only PASS findings on the clean calendar file."""
    fired = [
...
def test_every_rule_reports_something_on_the_clean_file(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean calendar file"
    )


def test_clean_file_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole calendar file rolls up to PASS."""
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~filing_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same calendar file twice yields findings in the same order."""
