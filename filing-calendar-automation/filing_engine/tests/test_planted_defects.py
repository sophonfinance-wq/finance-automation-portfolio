...
from filing_engine.engine import REGISTRY
from filing_engine.generate import DEFECTS
from filing_engine.model import DocumentReport, Status, Verdict
...
@pytest.mark.parametrize("name", defect_names())
def test_defect_fires_only_its_intended_rule(
    name: str, by_defect: dict[str, DocumentReport]
) -> None:
    """A defect confined to one base fact must trip exactly one control.

    Because every rollup re-derives from the base facts through the shared kernel,
    a single planted change stays contained to the control it targets -- it does
    not ripple into a second finding from a stale summary.
    """
    intended, _mutation = DEFECTS[name]
    assert by_defect[name].rules_fired() == [intended], (
        f"planted defect {name!r} tripped {by_defect[name].rules_fired()}, "
        f"expected only {[intended]}"
    )


@pytest.mark.parametrize("name", defect_names())
def test_defect_file_is_not_clean(
    name: str, by_defect: dict[str, DocumentReport]
) -> None:
    """A calendar file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    The bad fixed amount trips ``pay_fixed_amount`` and nothing else, and the
    structural precondition still ran to a PASS -- so the malformed value was
    contained to the one control that read it, not allowed to abort the file.
    """
    report = by_defect["amount_not_integer"]
    assert "pay_fixed_amount" in report.rules_fired()
    assert any(
        f.rule == "set_complete" and f.status is Status.PASS for f in report.findings
    )
