"""Shared fixtures for the filing-obligation calendar engine test suite.
...
from filing_engine.engine import analyze_document, analyze_folder
from filing_engine.generate import DEFECTS, generate_corpus
from filing_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("calendar_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every calendar file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one calendar file with no planted defect."""
