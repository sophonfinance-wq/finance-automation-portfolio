"""
Command-line interface for the Filing-obligation calendar engine.
=================================================================
...
::
    ...
    python -m filing_engine <folder> [options]

Analyzes every ``.json`` calendar file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m filing_engine",
        description=(
            "Read-only control engine for a fiscal-year filing-obligation calendar: every entity's return and payment obligation proved filed or validly extended before its statutory due date, each fixed-amount payment made in the exact statutory amount, and the status register tied to the filed evidence. Analyzes seeded, fictional calendar files."
        ),
    )
    p.add_argument("folder", help="folder containing .json calendar files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional calendar file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read calendar files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json calendar files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} calendar file(s))")
