"""
Command-line interface for the capital spending request gate engine.
====================================================================
...
::
    ...
    python -m spending_engine <folder> [options]

Analyzes every ``.json`` request package in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m spending_engine",
        description=(
            "Read-only control engine for a developer's capital Spending Request phase gates: hard dollar triggers proved to have fired before capital was committed, the five IC gates entered in sequence with prior-phase deliverables complete, and the underwriting standards (contingency floors, growth default, fee formulas, comparison basis) re-derived and tied. Analyzes seeded, fictional request packages."
        ),
    )
    p.add_argument("folder", help="folder containing .json request packages")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional request package(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read request packages in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json request packages found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} request package(s))")
