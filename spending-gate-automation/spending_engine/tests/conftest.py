"""Shared fixtures for the spending request gate engine test suite.
...
from spending_engine.engine import analyze_document, analyze_folder
from spending_engine.generate import DEFECTS, generate_corpus
from spending_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("request_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every request package in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one request package with no planted defect."""
