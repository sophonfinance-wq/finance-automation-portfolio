...
from spending_engine.engine import REGISTRY
from spending_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A request package carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad fee is read by more than one rule -- the CM-fee re-derivation
    and the total-project-cost footing -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "fee_cm_rederives" in fired
    assert "fee_tpc_foots" in fired
