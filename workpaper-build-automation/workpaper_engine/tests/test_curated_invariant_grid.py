"""Curated invariant grid -- exact identities over the shared money kernel.

Every assertion below is an exact equality checked across a bounded integer-cent
domain, so it gates CI on every push rather than living behind a sweep flag.
These are the arithmetic identities the whole build reduces to: a split that
conserves its total, a ratio allocation that neither loses nor mints a cent
however the remainder falls, and a rate applied as exact truncating integer
math. The member income rows, the tax income column and the liability shares are
all produced by ``allocate_by_ratio``, so its conservation property is swept over
the shipped 35/65 sharing ratio as itself rather than only spot-checked.
"""
...
# positive totals across one, two and three parts. 1,950 totals x 3 = 5,850.
# --------------------------------------------------------------------------- #
_SPLIT_GRID = list(itertools.product(range(-975, 975), range(1, 4)))
...
# The first shape is the sharing ratio the shipped allocation register carries,
# so the members' capital tie-out identity is swept as itself.
# --------------------------------------------------------------------------- #
_WEIGHT_SHAPES: tuple[tuple[int, ...], ...] = ((3500, 6500), (2500, 2500, 5000))
_ALLOC_GRID = list(itertools.product(range(0, 1000), _WEIGHT_SHAPES))


@pytest.mark.parametrize("total_cents,weights", _ALLOC_GRID)
def test_allocate_by_ratio_conserves_total(
