"""
Money primitives for the tax workpaper build engine.
====================================================

Every monetary amount inside the engine is an **integer number of cents**, so
arithmetic is exact and a constructed schedule ties to the source it was built
from with no rounding story. Floating-point dollars are never used for
accumulation or comparison; they appear only at the formatting edge. Dataclass
fields and JSON keys carry a ``_cents`` suffix.

Comparisons are exact ``==`` -- there is **no tolerance band**. A workpaper cell
that tolerates a penny against the ledger beneath it reports a figure the ledger
does not support, and because this engine *builds* the workpaper rather than
merely reading it, that penny is then carried forward into every future year's
frozen layer. A column that "approximately" foots to zero is a column with an
unmapped account in it.
...
* :func:`allocate_by_ratio` splits an amount by integer basis-point weights using
  the largest-remainder method, guaranteeing the parts sum to the whole. This is
  the one the member income allocation stands on: a split that mints or loses a
  cent puts the members' capital accounts permanently out of agreement with the
  equity schedule above them.

Two more round out the shared contract:

* :func:`apply_rate` applies an integer basis-point rate with truncating
  division, so a figure derived as a rate on a base can be compared to the
  reported figure with exact ``==``.
* :func:`percent_complete_bps` derives a ratio as integer basis points, floored,
  used wherever a consumed-against-target proportion is reported rather than
  acted on.
...
reports that code rather than coercing the amount: silently rounding a figure
would make the engine the author of the number it is meant to prove.
"""
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"equity[EQ-2029-C-3000]/amount_cents"``.
    value:
...
def require_cents(field: str, value: object, *, unit: str = "integer cents") -> int:
    ...
    Args:
        ...
        unit: What the field is expected to hold, quoted in the message. The
            same guard protects amounts, whole counts and basis-point splits.
...
def fmt_bps(bps: int) -> str:
    """Format integer basis points as a percentage string (``3500`` -> ``35.00%``)."""
    sign = "-" if bps < 0 else ""
...
def split_evenly(total_cents: int, parts: int) -> list[int]:
    """Split a total into ``parts`` equal integer-cent pieces.

    The pieces sum exactly to ``total_cents``; any rounding remainder is added to
    the **final** piece so straight-line schedules never drift.

    Args:
        total_cents: Amount to split, in cents.
        parts: Number of pieces (must be >= 1).

    Returns:
        A list of ``parts`` integer-cent amounts summing to ``total_cents``.
...
def allocate_by_ratio(total_cents: int, weights_bps: Sequence[int]) -> list[int]:
    ...
    exactly to ``total_cents`` with no penny lost or created. The member income
    rows on the equity schedule and the income column of every member capital
    account are produced here, which is why the conservation property is swept
...
def apply_rate(base_cents: int, rate_bps: int) -> int:
    ...
    Truncating division is the contract: a figure derived elsewhere as a rate on
    a base is derived the same way, so the engine can compare the reported figure
    to the derived one with exact ``==``.

    Args:
        base_cents: Base amount in cents.
        rate_bps: Rate in basis points (3500 == 35.00%).
...
def percent_complete_bps(actual_cents: int, budget_cents: int) -> int:
    """Progress against a target, in integer basis points, floored at zero.

    Floor division is deliberate. The result is reported, not acted on -- it says
    how much of a target has been consumed -- and flooring keeps the engine from
    ever overstating consumption by a rounding artifact.

    A zero or negative budget returns ``0``: no target means no ratio to report. A
    negative actual also returns ``0`` rather than a negative ratio.

    Args:
        actual_cents: Amount consumed to date.
        budget_cents: The target it is measured against.

    Returns:
        Percent consumed in basis points (``3500`` == 35.00%).
    """
