...
from rollforward_engine.engine import analyze_document, analyze_folder
from rollforward_engine.generate import generate_corpus
from rollforward_engine.report import build_json_report, build_markdown_report
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """A workpaper file with no documents fails loudly rather than passing vacuously."""
...
def test_no_control_reads_the_system_clock(corpus: Path) -> None:
    """Findings depend only on the file's own fiscal window.

    A control that reached for today's date would make the corpus pass or fail
    according to when CI happened to run. The fiscal window is fictional and in
    the future, so any clock-reading rule would drift the moment the machine's
    date moved.
    """
    first = [r.to_dict() for r in analyze_folder(corpus)]
    # Re-analyzing after the wall clock has advanced within the run must not
    # change a single finding; the corpus carries its own period.
    second = [r.to_dict() for r in analyze_folder(corpus)]
    assert first == second
    md = build_markdown_report(analyze_folder(corpus))
    assert "never the system clock" in md
