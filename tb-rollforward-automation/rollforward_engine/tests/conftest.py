"""Shared fixtures for the trial-balance roll-forward engine test suite.
...
from rollforward_engine.generate import DEFECTS, generate_corpus
from rollforward_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("rollforward_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every workpaper file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one workpaper file with no planted defect."""
