"""
Command-line interface for the trial-balance roll-forward engine.
=================================================================
...
::
    ...
    python -m rollforward_engine <folder> [options]

Analyzes every ``.json`` workpaper file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m rollforward_engine",
        description=(
            "Read-only control engine for the annual trial-balance roll-forward: last year's column structure carried forward, final-year entities retired, renames carried once, first-year entities admitted with their registration dates, and every column repopulated from the year-end extract's declared balance column and tied cell by cell to an embedded backup tab. Analyzes seeded, fictional workpaper files."
        ),
    )
    p.add_argument("folder", help="folder containing .json workpaper files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional workpaper file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read workpaper files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json workpaper files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} workpaper file(s))")
