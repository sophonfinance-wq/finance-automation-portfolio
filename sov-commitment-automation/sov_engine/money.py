"""
Money primitives for the subcontract SOV & change orders engine.
================================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. Completed to
date is previously billed plus this period, and payment due is the request less
retention plus tax -- identities, not approximations, so a penny of tolerance
means a subcontractor is certified for an amount the schedule of values does not
authorize. Retention and sales tax are struck with truncating basis-point
arithmetic, so the contract rate and the billed figure can be compared exactly.
...
* :func:`allocate_by_ratio` splits an amount by integer basis-point weights using
  the largest-remainder method, guaranteeing the parts sum to the whole.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"allocation[BR-2029/PRJ-114]/allocated_cents"``.
    value:
