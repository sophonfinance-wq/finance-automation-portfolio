...
from sov_engine.engine import REGISTRY
from sov_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A billing file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad retention figure is read by more than one rule -- the retention
    re-derivation and the payment-due re-derivation -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "ret_rederives" in fired
    assert "tax_payment_due_rederives" in fired
