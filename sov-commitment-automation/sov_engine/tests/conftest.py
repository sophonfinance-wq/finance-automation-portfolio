"""Shared fixtures for the subcontract SOV & change-order engine test suite.
...
from sov_engine.engine import analyze_document, analyze_folder
from sov_engine.generate import DEFECTS, generate_corpus
from sov_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("billing_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every billing file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one billing file with no planted defect."""
