"""
Command-line interface for the subcontract SOV & change-order engine.
=====================================================================
...
::
    ...
    python -m sov_engine <folder> [options]

Analyzes every ``.json`` billing file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m sov_engine",
        description=(
            "Read-only control engine for a residential developer's subcontract commitment billing: every schedule of values footed to its contract, every revised commitment re-derived as original + net approved change orders, every pay-app line, cap, retention, tax and roll-forward recomputed, and every conditional lien release tied to the certified payment. Analyzes seeded, fictional billing files."
        ),
    )
    p.add_argument("folder", help="folder containing .json billing files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional billing file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read billing files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json billing files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} billing file(s))")
