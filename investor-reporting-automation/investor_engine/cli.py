"""
Command-line interface for the JV investor reporting engine.
============================================================
...
::
    ...
    python -m investor_engine <folder> [options]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m investor_engine",
        description=(
            "Read-only control engine for the monthly financial reporting package a developer issues to the joint-venture equity partner of a single project entity: the trial balance, balance sheet, statement of operations, statement of members' capital, development cost schedule, job cost bridge and notes, re-derived from the project entity's ledger, the job cost detail, the executed equity draw schedule, the parent's reciprocal investment account and the prior period's issued package. Every difference is quantified and the package is held until each one is cleared or disclosed. Analyzes seeded, fictional package files."
        ),
    )
    p.add_argument("folder", help="folder containing .json package files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional package file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read package files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json package files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} package file(s))")
