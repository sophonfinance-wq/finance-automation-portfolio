"""
Money primitives for the JV investor reporting engine.
======================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A trial balance
that nets to a penny does not net: the package built on it carries a difference
that has to land somewhere, and wherever it lands is a figure no longer supported
by the ledger. A statement either ties to the account behind it to the cent or it
does not tie at all.
...
* :func:`allocate_by_ratio` splits an amount by integer basis-point weights using
  ...
  the one that matters here: a net income allocated between members by sharing
  ratio must add back to the income statement bottom line, or the capital
  statement's Total column stops cross-footing.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"trial_balance[2010]/debit_cents"``.
    value:
...
def allocate_by_ratio(total_cents: int, weights_bps: Sequence[int]) -> list[int]:
    ...
    exactly to ``total_cents`` with no penny lost or created. That exactness is
    the whole point when the parts are member columns and the total is a printed
    row: a penny created here is a Total column that stops cross-footing.
