...
from investor_engine.money import (
    AMOUNT_INVALID,
    BPS_FULL,
    AmountInvalidError,
    allocate_by_ratio,
...
def test_require_cents_reports_the_unit_it_expected() -> None:
    with pytest.raises(AmountInvalidError) as exc:
        require_cents("ownership", 1.5, unit="integer basis points")
...
@pytest.mark.parametrize(
    "bps,expected",
    [(0, "0.00%"), (10000, "100.00%"), (2000, "20.00%"), (8000, "80.00%"),
     (-250, "-2.50%")],
)
...
@pytest.mark.parametrize(
    "amount,weights",
    [
        # The shipped sharing ratio is 20/80, so it is swept as itself.
        (100, [2000, 8000]), (100, [3333, 3333, 3334]), (1, [2000, 8000]),
...
def test_allocate_by_ratio_rejects_bad_weights() -> None:
    with pytest.raises(ValueError, match="10000"):
        allocate_by_ratio(100, [2000, 7000])
...
def test_allocating_a_result_between_members_cross_foots() -> None:
    """A result split by sharing ratio has to add back to the bottom line.

    This is the identity the capital statement's Total column stands on: allocate
    an odd amount 20/80 and the two member columns must still sum to the figure
    the statement of operations reports, to the cent.
    """
    parts = allocate_by_ratio(45_000_01, [2000, 8000])
    assert sum(parts) == 45_000_01
    assert len(parts) == 2
