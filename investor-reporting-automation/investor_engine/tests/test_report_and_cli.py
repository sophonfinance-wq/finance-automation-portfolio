...
from investor_engine.cli import main
from investor_engine.engine import REGISTRY, analyze_folder
from investor_engine.model import DocumentReport, Status, Verdict
from investor_engine.report import (
    build_json_report,
...
def test_markdown_states_the_period_comes_from_the_file(
    reports: list[DocumentReport],
) -> None:
    """A draw tested against the system clock would report differently tomorrow.

    The report says the period comes from the file, and that claim is the reason
    the committed output is diffable at all, so it is asserted rather than
    trusted.
    """
