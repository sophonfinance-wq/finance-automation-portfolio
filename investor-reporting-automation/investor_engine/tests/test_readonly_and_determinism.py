...
from investor_engine.engine import analyze_document, analyze_folder
from investor_engine.generate import generate_corpus
from investor_engine.report import build_json_report, build_markdown_report
...
def test_analysis_does_not_mutate_the_parsed_payload(corpus: Path) -> None:
    """The context is a view, not a copy, so a check that wrote would show here.

    Read-only over the file on disk is not enough on its own: a check that edited
    the parsed payload in memory would leave the bytes alone and still corrupt
    every control that ran after it. Analyzing a file, then re-reading it and
    analyzing again, has to produce the same findings in the same order.
    """
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """A package file with no documents fails loudly rather than passing vacuously."""
    p = tmp_path / "empty.json"
    p.write_text("{}", encoding="utf-8")
    report = analyze_document(p)
    assert "package_complete" in report.rules_fired()


def test_an_absent_document_never_produces_a_pass(tmp_path: Path) -> None:
    """A control whose evidence is missing reports nothing, not a clean result.

    This is the property that keeps ``package_complete`` load-bearing: a package
    with nothing in it must not come back with twenty-four passing controls and
    one failure, because a PASS on evidence that is not there is worse than no
    report at all.
    """
    p = tmp_path / "empty.json"
    p.write_text("{}", encoding="utf-8")
    report = analyze_document(p)
    assert {f.rule for f in report.findings} == {"package_complete"}
