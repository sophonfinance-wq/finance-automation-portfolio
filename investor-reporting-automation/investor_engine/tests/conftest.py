"""Shared fixtures for the JV investor reporting package engine test suite.
...
from investor_engine.generate import DEFECTS, generate_corpus
from investor_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("investor_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every package file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one package file with no planted defect."""
