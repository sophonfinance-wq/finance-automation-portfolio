...
from investor_engine.engine import REGISTRY
from investor_engine.generate import DEFECTS
from investor_engine.model import DocumentReport, Status, Verdict
...
def test_defect_package_is_not_clean(
    ...
    """A package file carrying a planted defect never rolls up to PASS."""
...
def test_each_defect_is_confined_to_its_own_control(
    ...
    of the three is independently violable -- and the release gate would then be
    held by findings nobody designed. The malformed-amount file is the one
    deliberate exception: a bad figure is read by more than one control and every
    one of them has to report it rather than abort.
    """
    for name, report in sorted(by_defect.items()):
        if name in ("clean", "amount_not_integer"):
            continue
        intended, _mutation = DEFECTS[name]
...
def test_flag_only_defects_roll_up_to_review(
    ...
    The one FLAG-severity control ships a defect that fires nothing else, so the
    ...
    """
    for name in ("cash_support_absent",):
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad trial balance debit is read by more than one rule -- the netting
    control and the agreement-to-ledger control -- and both must still report
    rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "tb_nets_zero" in fired
    assert "tb_agrees_ledger" in fired


def test_the_gate_holds_every_defect_package(
    by_defect: dict[str, DocumentReport]
) -> None:
    """No defect file leaves the building.

    Every defect file but the gate's own is marked held before its mutation is
    applied, so the gate agrees with the control position it is in and stays
    silent. The gate's own file is marked released over an open exception, which
    is the defect. Either way, no package carrying a planted break is presented
    as issued and clean.
    """
    for name, report in sorted(by_defect.items()):
        if name == "clean":
            continue
        if name == "released_uncleared":
            assert "release_gate" in report.rules_fired()
        else:
            assert "release_gate" not in report.rules_fired(), (
                f"{name} tripped the release gate as well as its own control"
            )
