"""
Command-line interface for the information-return issuance engine.
=================================================================
...
::
    ...
    python -m inforeturn_engine <folder> [options]

Analyzes every ``.json`` reporting file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m inforeturn_engine",
        description=(
            "Read-only control engine for the annual information-return issuance run: the payee population reassembled entity by entity, the box threshold applied to it, a form issued exactly where one is owed, backup withholding recomputed at the statutory rate, and every box footed back to the payment lines it came from. Analyzes seeded, fictional reporting files."
        ),
    )
    p.add_argument("folder", help="folder containing .json reporting files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional reporting file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read reporting files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json reporting files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} reporting file(s))")
