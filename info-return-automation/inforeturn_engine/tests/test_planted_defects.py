...
from inforeturn_engine.engine import REGISTRY
from inforeturn_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A reporting file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad box amount is read by more than one rule -- the tie-out to the
    ledger and the backup-withholding recompute that sits on the same form -- and
    both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "frm_box_ties_ledger" in fired
    assert "bwh_rate_recomputes" in fired
