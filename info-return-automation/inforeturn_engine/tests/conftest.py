"""Shared fixtures for the information-return issuance engine test suite.
...
from inforeturn_engine.engine import analyze_document, analyze_folder
from inforeturn_engine.generate import DEFECTS, generate_corpus
from inforeturn_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("inforeturn_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every reporting file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one reporting file with no planted defect."""
