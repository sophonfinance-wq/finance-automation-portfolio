"""
Money primitives for the surety bond & collateral engine.
=========================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A collateral
balance that tolerates a penny against the postings that built it has not been
reconciled; it has been rounded until it looked reconciled, which is the opposite
thing. Collateral is cash a surety is holding, and the entire reason to track it
is that it has to come back.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"bond[WA-0007]/collateral_posted_cents"``.
    value:
