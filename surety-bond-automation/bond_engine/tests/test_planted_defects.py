...
from bond_engine.engine import REGISTRY
from bond_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A programme file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    readable. A single bad collateral amount is read by more than one rule -- the
    bond's own tie-out to the ledger and the programme-wide tie to restricted cash
    -- and both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "coll_ledger_ties_register" in fired
    assert "coll_ties_restricted_cash_gl" in fired
