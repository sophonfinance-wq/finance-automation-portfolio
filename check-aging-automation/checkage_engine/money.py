"""
Money primitives for the outstanding check aging & escheatment engine.
======================================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. An outstanding
balance that tolerates a penny against the register it is derived from is
exactly the drift the bank reconciliation then ties to. A check is a fixed sum
somebody is still owed, and the amount that eventually escheats to a state is
that sum, not a rounded version of it.
