"""
Command-line interface for the outstanding-check aging engine.
==============================================================
...
::
    ...
    python -m checkage_engine <folder> [options]

Analyzes every ``.json`` aging file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m checkage_engine",
        description=(
            "Read-only control engine for outstanding-check aging and escheatment: the outstanding set re-derived from the check register, aged to the as-of date, tied to the bank reconciliation, and tested for stale-dating and dormancy. Analyzes seeded, fictional aging files."
        ),
    )
    p.add_argument("folder", help="folder containing .json aging files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional aging file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read aging files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json aging files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} aging file(s))")
