...
from checkage_engine.engine import REGISTRY
from checkage_engine.generate import DEFECTS
from checkage_engine.model import DocumentReport, Status, Verdict
...
def test_defect_package_is_not_clean(
    ...
    """An aging file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad check amount is read by more than one rule -- the outstanding-set
    recompute and the bank-reconciliation tie-out -- and both must still report
    rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "age_outstanding_set_recomputes" in fired
    assert "rpt_outstanding_total_ties" in fired


def test_a_withdrawn_check_on_the_schedule_is_owned_by_the_void_control(
    by_defect: dict[str, DocumentReport]
) -> None:
    """The set-recompute control defers to ``void_not_outstanding`` on that row.

    Two controls that both fired on one row would double-report the same break and
    make the exception count meaningless, so the ownership boundary is asserted
    rather than assumed.
    """
    fired = by_defect["void_listed_outstanding"].rules_fired()
    assert fired == ["void_not_outstanding"], fired
