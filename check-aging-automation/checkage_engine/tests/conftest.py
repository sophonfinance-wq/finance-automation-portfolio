"""Shared fixtures for the outstanding-check aging engine test suite.
...
from checkage_engine.generate import DEFECTS, generate_corpus
from checkage_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("aging_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every aging file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one aging file with no planted defect."""
