...
and a participant in a disbursement process is the one thing this engine must
never be.
"""
...
from checkage_engine.engine import analyze_document, analyze_folder
from checkage_engine.generate import generate_corpus
from checkage_engine.report import build_json_report, build_markdown_report
...
def test_ages_are_measured_to_the_file_not_the_clock(corpus: Path) -> None:
    """No age in the corpus moves because a day passed on the build machine.

    An engine that read the system clock would produce a different aging profile
    every night, which is the one thing a committed, diffable report cannot
    survive.
    """
    first = [r.to_dict() for r in analyze_folder(corpus)]
...
def test_empty_package_does_not_crash(tmp_path: Path) -> None:
    """An aging file with no documents fails loudly rather than passing vacuously."""
