...
from checkage_engine.cli import main
from checkage_engine.engine import REGISTRY, analyze_folder
from checkage_engine.model import DocumentReport, Status, Verdict
from checkage_engine.report import (
    build_json_report,
...
def test_markdown_states_the_engine_never_acts(
    reports: list[DocumentReport],
) -> None:
    """A control engine over a disbursement process must say what it will not do."""
    md = build_markdown_report(reports).lower()
    assert "read-only" in md
    assert "escheat" in md
