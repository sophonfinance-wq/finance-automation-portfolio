"""
Command-line interface for the non-resident withholding engine.
===============================================================
...
::
    ...
    python -m withholding_engine <folder> [options]

Analyzes every ``.json`` withholding file in ``<folder>`` (read-only) and prints
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m withholding_engine",
        description=(
            "Read-only control engine for a US withholding agent's non-resident (Chapter 3/4 FDAP) withholding and Form 1042 / 1042-S reconciliation: every payment rated from the payee's status and the treaty table, every reduced rate supported by a valid W-8, the deposits tied to the liability, and the 1042 tied to the slips. Analyzes seeded, fictional withholding files."
        ),
    )
    p.add_argument("folder", help="folder containing .json withholding files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional withholding file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read withholding files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json withholding files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} withholding file(s))")
