...
from withholding_engine.cli import main
from withholding_engine.engine import REGISTRY, analyze_folder
from withholding_engine.model import DocumentReport, Status, Verdict
from withholding_engine.report import (
    build_json_report,
...
def test_markdown_report_has_structure(reports: list[DocumentReport]) -> None:
    ...
    assert "## Withholding files" in md
    assert md.endswith("\n")
...
def test_markdown_lists_every_file(reports: list[DocumentReport]) -> None:
    md = build_markdown_report(reports)
    for report in reports:
