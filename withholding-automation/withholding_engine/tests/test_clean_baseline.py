"""The clean withholding file must satisfy every control, rule by rule.
...
from withholding_engine.engine import REGISTRY, SEVERITY, analyze_document
from withholding_engine.model import DocumentReport, Status, Verdict

RULE_IDS = [rule_id for rule_id, _fn in REGISTRY]


@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_clean_file_passes_rule(rule_id: str, clean_report: DocumentReport) -> None:
    """Each control produces only PASS findings on the clean withholding file."""
    fired = [
...
@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_every_rule_reports_something_on_the_clean_file(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean withholding file"
    )


def test_clean_file_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole withholding file rolls up to PASS."""
    assert clean_report.verdict is Verdict.PASS


def test_clean_file_has_no_flags(clean_report: DocumentReport) -> None:
...
def test_registry_is_not_empty() -> None:
    ...
    The floor is below the count this engine actually ships, so a rule quietly
    failing to register shows up here rather than as a corpus that mysteriously
    passes.
    """
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    registered rule must have declared its severity -- a rule that returned early
    before declaring one would be invisible to
    :func:`~withholding_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same withholding file twice yields findings in the same order."""
