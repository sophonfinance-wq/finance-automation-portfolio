"""Shared fixtures for the non-resident withholding engine test suite.
...
from withholding_engine.engine import analyze_document, analyze_folder
from withholding_engine.generate import DEFECTS, generate_corpus
from withholding_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("withholding_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every withholding file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one withholding file with no planted defect."""
