...
from withholding_engine.engine import REGISTRY
from withholding_engine.generate import DEFECTS
...
def test_defect_file_is_not_clean(
    ...
    """A withholding file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    The corrupted gross is read by more than one rule -- the tax recompute and the
    slip aggregation behind the tie-outs -- and the file still produces findings
    from the rules that do not touch it rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "wh_tax_recomputes" in fired
    # The clean-passing structural controls still ran and reported.
    passing = {f.rule for f in report.findings if f.status is Status.PASS}
    assert "set_complete" in passing
    assert "payee_unique_ids" in passing
