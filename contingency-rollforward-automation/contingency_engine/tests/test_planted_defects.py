...
from contingency_engine.engine import REGISTRY
from contingency_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A period file carrying a planted defect never rolls up to PASS."""
...
def test_flag_only_defects_roll_up_to_review(
    ...
    """
    for name in ("headroom_watch", "watchlist_overstated"):
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad current balance is read by more than one rule -- the rollforward
    identity and the adequacy recompute -- and both must still report rather than
    aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "ctg_current_rolls_forward" in fired
    assert "adq_flag_recomputes" in fired
