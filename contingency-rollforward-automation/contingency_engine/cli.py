"""
Command-line interface for the contingency rollforward engine.
==============================================================
...
::
    ...
    python -m contingency_engine <folder> [options]

Analyzes every ``.json`` period file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m contingency_engine",
        description=(
            "Read-only control engine for the contingency status block in a developer's periodic project report: construction and project contingency rolled forward from prior balance through the draws allocated against them, reconciled to the budget that funded them, and assessed adequate only when the balance remaining covers the projected potential use. Analyzes seeded, fictional period files."
        ),
    )
    p.add_argument("folder", help="folder containing .json period files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional period file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read period files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json period files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} period file(s))")
