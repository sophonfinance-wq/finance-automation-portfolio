"""
Command-line interface for the interest-accrual & loan-amortization engine.
===========================================================================
...
::
    ...
    python -m interest_engine <folder> [options]

Analyzes every ``.json`` loan file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m interest_engine",
        description=(
            "Read-only control engine for a lender's interest accrual and loan amortization: every note's accrued interest re-derived from its terms, its balance rolled forward begin to end, its maturity/rate/prepayment/subordination gates enforced, and its reciprocal GL and journal tied out. Analyzes seeded, fictional loan files."
        ),
    )
    p.add_argument("folder", help="folder containing .json loan files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional loan file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read loan files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json loan files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} loan file(s))")
