...
from interest_engine.engine import REGISTRY
from interest_engine.generate import DEFECTS
...
def test_defect_file_is_not_clean(
    ...
    """A loan file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad beginning balance is read by more than one rule -- the accrual
    re-derivation and the roll-forward identity -- and both must still report
    rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "acc_interest_rederives" in fired
    assert "rf_ending_balance_rederives" in fired
