"""Shared fixtures for the interest-accrual engine test suite.
...
from interest_engine.engine import analyze_document, analyze_folder
from interest_engine.generate import DEFECTS, generate_corpus
from interest_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("loan_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every loan file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one loan file with no planted defect."""
