"""
Command-line interface for the depreciation & prepaid amortization engine.
==========================================================================
...
::
    ...
    python -m depreciation_engine <folder> [options]

Analyzes every ``.json`` close file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m depreciation_engine",
        description=(
            "Read-only control engine for an entity's fixed-asset depreciation register and prepaid amortization schedule: every straight-line charge, months-used count, accumulated balance, NBV and prepaid waterfall figure re-derived from cost, life, convention and the stated window, the roll-forwards closed, and the register totals tied to the GL control accounts and the recurring JE. Analyzes seeded, fictional close files."
        ),
    )
    p.add_argument("folder", help="folder containing .json close files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional close file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read close files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json close files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} close file(s))")
