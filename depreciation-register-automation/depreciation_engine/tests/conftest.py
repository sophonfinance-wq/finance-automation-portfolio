"""Shared fixtures for the depreciation & prepaid amortization engine test suite.
...
from depreciation_engine.engine import analyze_document, analyze_folder
from depreciation_engine.generate import DEFECTS, generate_corpus
from depreciation_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("close_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every close file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one close file with no planted defect."""
