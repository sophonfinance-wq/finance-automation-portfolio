...
from depreciation_engine.engine import REGISTRY
from depreciation_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A close file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad accumulated balance is read by more than one rule -- the
    accumulated re-derivation, the NBV identity, the roll-forward footing and the
    GL tie-out -- and all of them must still report rather than aborting at the
    first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "nbv_accum_rederives" in fired
    assert "roll_closing_ties_register" in fired
    assert "gl_accum_ties" in fired
