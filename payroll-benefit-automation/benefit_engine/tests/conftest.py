"""Shared fixtures for the payroll & benefit reconciliation engine test suite.
...
from benefit_engine.engine import analyze_document, analyze_folder
from benefit_engine.generate import DEFECTS, generate_corpus
from benefit_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("reconciliation_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every reconciliation file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one reconciliation file with no planted defect."""
