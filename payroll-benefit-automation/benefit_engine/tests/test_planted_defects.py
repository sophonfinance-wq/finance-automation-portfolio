...
from benefit_engine.engine import REGISTRY
from benefit_engine.generate import DEFECTS
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad deduction is read by more than one rule -- the provider tie-out
    and the plan-tie recompute -- and both must still report rather than aborting
    at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "tie_provider_total" in fired
    assert "rpt_tie_status_recomputes" in fired
