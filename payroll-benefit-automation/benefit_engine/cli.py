"""
Command-line interface for the payroll & benefit reconciliation engine.
=======================================================================
...
::
    ...
    python -m benefit_engine <folder> [options]

Analyzes every ``.json`` reconciliation file in ``<folder>`` (read-only) and
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m benefit_engine",
        description=(
            "Read-only control engine for one pay period's payroll benefit reconciliation: every 401(k), HSA and FSA deduction proved to reach each provider, the ledger and cash in the same amount, the employer match recomputed, every statutory limit respected and every deposit inside the DOL safe harbor. Analyzes seeded, fictional reconciliation files."
        ),
    )
    p.add_argument("folder", help="folder containing .json reconciliation files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional reconciliation file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read reconciliation files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json reconciliation files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} reconciliation file(s))")
