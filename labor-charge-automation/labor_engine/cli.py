"""
Command-line interface for the project labor charge engine.
===========================================================
...
::
    ...
    python -m labor_engine <folder> [options]

Analyzes every ``.json`` charge-run file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m labor_engine",
        description=(
            "Read-only control engine for charging employee labor cost to projects: "
            "advance authorizations, monthly charges matched to them, invoice footing, "
            "budget overrun and general-ledger tie-out. Analyzes seeded, fictional "
            "charge-run files."
        ),
    )
    p.add_argument("folder", help="folder containing .json charge-run files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional charge-run file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read charge-run files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json charge-run files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} charge-run file(s))")
