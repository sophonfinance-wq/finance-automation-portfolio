"""The clean charge-run file must satisfy every control, rule by rule.
...
from labor_engine.engine import REGISTRY, SEVERITY, analyze_document
from labor_engine.model import DocumentReport, Status, Verdict
...
def test_clean_package_passes_rule(
    ...
    """Each control produces only PASS findings on the clean charge-run file."""
    fired = [
...
def test_every_rule_reports_something_on_the_clean_package(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean charge-run file"
    )


def test_clean_package_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole charge-run file rolls up to PASS."""
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~labor_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same charge-run file twice yields findings in the same order."""
