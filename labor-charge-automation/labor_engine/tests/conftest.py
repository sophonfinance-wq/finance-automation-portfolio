"""Shared fixtures for the project labor charge engine test suite.
...
from labor_engine.engine import analyze_document, analyze_folder
from labor_engine.generate import DEFECTS, generate_corpus
from labor_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("charge_run_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every charge-run file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one charge-run file with no planted defect."""
