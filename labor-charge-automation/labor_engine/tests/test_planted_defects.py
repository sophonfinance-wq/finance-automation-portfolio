...
from labor_engine.engine import REGISTRY
from labor_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A charge-run file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad charge amount is read by more than one rule -- the charge's match
    to its authorization and the portfolio-wide tie to the general ledger -- and
    both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "chg_amount_matches_auth" in fired
    assert "gl_total_ties_ledger" in fired
