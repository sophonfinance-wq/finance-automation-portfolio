"""Shared fixtures for the debt term-sheet sizing engine test suite.
...
from sizing_engine.engine import analyze_document, analyze_folder
from sizing_engine.generate import DEFECTS, generate_corpus
from sizing_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("sizing_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every sizing file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one sizing file with no planted defect."""
