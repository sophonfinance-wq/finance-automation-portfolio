...
from sizing_engine.engine import REGISTRY
from sizing_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A sizing file carrying a planted defect never rolls up to PASS."""
...
def test_every_finding_carries_a_message(
    ...
    Messages are the product here: a treasurer reading the report should learn
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad max-loan figure is read by more than one rule -- the
    over-advance control and the portfolio commitment total -- and both must still
    report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "size_ltc_not_exceeded" in fired
    assert "rpt_commitment_total_ties" in fired
