"""
Command-line interface for the debt term-sheet sizing engine.
=============================================================
...
::
    ...
    python -m sizing_engine <folder> [options]

Analyzes every ``.json`` sizing file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m sizing_engine",
        description=(
            "Read-only control engine for a borrower's debt term-sheet sizing: every deal's max loan within the advance rate, all-in rate and fee amounts re-derived, term and extensions reconciled to maturity, and the portfolio rollup footed. Analyzes seeded, fictional sizing files."
        ),
    )
    p.add_argument("folder", help="folder containing .json sizing files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional sizing file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read sizing files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json sizing files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} sizing file(s))")
