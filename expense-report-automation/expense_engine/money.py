"""
Money primitives for the expense & P-Card control engine.
=========================================================
...
Comparisons are exact ``==`` -- there is **no tolerance band**. A report that
reconciles to its statement to within a penny has not reconciled; it has been
rounded until it looked reconciled, which is the opposite thing. An expense
control lives or dies on the residual cent: every charge belongs to exactly one
report line, and every report line ties its statement line to the cent.
...
class AmountInvalidError(ValueError):
    ...
    field:
        ...
        ``"statement[STMT-01/L-0101]/amount_cents"``.
    value:
