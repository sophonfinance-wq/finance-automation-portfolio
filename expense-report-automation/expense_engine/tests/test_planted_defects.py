...
from expense_engine.engine import REGISTRY
from expense_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A cycle file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad statement amount is read by more than one rule -- the statement's
    own integer-cents check and the reconciliation of that charge to its report
    line -- and both must still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "stmt_amounts_integer" in fired
    assert "rec_every_charge_reported" in fired
