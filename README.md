...
[![Systems](https://img.shields.io/badge/systems-11%20runnable-6f42c1)](#the-eleven-systems)
...
Eleven self-contained Python systems for finance and tax work — month-end close, cash/debt
...
# run the curated engine suite (2,480 hand-written tests, expanded to 70,303 with invariant grids; runs in minutes)
...
| ↳ expanded with invariant grids | *(same scoped `pytest` run)* | **70,303** | The hand-written tests parametrized over bounded integer domains (`itertools.product`), so each property is checked across many cases. |
| **Site tooling** (separate guard suite) | `pytest -m site_tooling` | **51** | Generator, schema, freshness, accessibility, and page-budget guards. Excluded from the 70,303 curated engine total. |
