"""Shared fixtures for the equity-pickup engine test suite.
...
from pickup_engine.engine import analyze_document, analyze_folder
from pickup_engine.generate import DEFECTS, generate_corpus
from pickup_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("consolidation_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every consolidation file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one consolidation file with no planted defect."""
