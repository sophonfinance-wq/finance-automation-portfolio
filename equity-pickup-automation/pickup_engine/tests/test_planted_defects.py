...
from pickup_engine.engine import REGISTRY
from pickup_engine.generate import DEFECTS
from pickup_engine.model import DocumentReport, Status, Verdict
...
def test_defect_package_is_not_clean(
    ...
    """A consolidation file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad pickup figure is read by more than one rule -- the roll-forward
    identity and the pickup re-derivation -- and both must still report rather
    than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "rf_identity" in fired
    assert "epu_pickup_rederives" in fired


def test_the_flag_only_defect_rolls_up_to_review(
    by_defect: dict[str, DocumentReport]
) -> None:
    """The unposted-adjustment file carries FLAGs and nothing harder.

    That is the REVIEW verdict's whole meaning: worth a human's eyes, not a hard
    stop -- and it proves the FAIL/FLAG split is wired through the roll-up.
    """
    report = by_defect["tb_unposted_adjustment"]
