"""
Command-line interface for the equity-pickup control engine.
============================================================
...
::
    ...
    python -m pickup_engine <folder> [options]

Analyzes every ``.json`` consolidation file in ``<folder>`` (read-only) and
prints the exceptions. Optionally writes the full markdown and JSON reports.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m pickup_engine",
        description=(
            "Read-only control engine for equity-method pickup & consolidation eliminations: every parent's investment roll-forward re-footed, every pickup re-derived as ownership x sub net income, every Investment account paired to its sub's Equity-Parent account and extinguished, and the preferred-return accrual, member allocation and trial balance tied out exactly. Analyzes seeded, fictional consolidation files."
        ),
    )
    p.add_argument("folder", help="folder containing .json consolidation files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional consolidation file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read consolidation files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json consolidation files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} consolidation file(s))")
