"""
Command-line interface for the buyer upgrade engine.
===================================================
...
::
    ...
    python -m upgrade_engine <folder> [options]

Analyzes every ``.json`` upgrade book in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m upgrade_engine",
        description=(
            "Read-only control engine for buyer upgrade revenue and cost. "
            "Analyzes seeded, fictional upgrade books."
        ),
    )
    p.add_argument("folder", help="folder containing .json upgrade books")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional upgrade book(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read upgrade books in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json upgrade books found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} upgrade book(s))")
