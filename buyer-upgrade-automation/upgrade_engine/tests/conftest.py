...
from upgrade_engine.engine import analyze_document, analyze_folder
from upgrade_engine.generate import DEFECTS, generate_corpus
from upgrade_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("upgrade_corpus")
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one book with no planted defect."""
