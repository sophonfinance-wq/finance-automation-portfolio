...
from closing_engine.engine import REGISTRY
from closing_engine.generate import DEFECTS
...
def test_defect_package_is_not_clean(
    ...
    """A closing file carrying a planted defect never rolls up to PASS."""
...
def test_amount_invalid_is_contained_to_its_row(
    ...
    A single bad broker commission is read by more than one rule -- the
    settlement-charges recompute and the unit tie-out recompute -- and both must
    still report rather than aborting at the first bad value.
    """
    report = by_defect["amount_not_integer"]
    fired = report.rules_fired()
    assert "ss_total_charges_recompute" in fired
    assert "rpt_unit_tied_recomputes" in fired
