"""Shared fixtures for the closing & settlement tie-out engine test suite.
...
from closing_engine.engine import analyze_document, analyze_folder
from closing_engine.generate import DEFECTS, generate_corpus
from closing_engine.model import DocumentReport


@pytest.fixture(scope="session")
def corpus(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """A freshly generated corpus in a temporary directory."""
    folder = tmp_path_factory.mktemp("closing_corpus")
...
def reports(corpus: Path) -> list[DocumentReport]:
    """Every closing file in the corpus, analyzed once."""
...
def clean_report(by_defect: dict[str, DocumentReport]) -> DocumentReport:
    """The one closing file with no planted defect."""
