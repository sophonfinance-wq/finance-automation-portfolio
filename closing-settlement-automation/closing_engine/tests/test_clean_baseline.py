"""The clean closing file must satisfy every control, rule by rule.
...
from closing_engine.engine import REGISTRY, SEVERITY, analyze_document
from closing_engine.model import DocumentReport, Status, Verdict

RULE_IDS = [rule_id for rule_id, _fn in REGISTRY]


@pytest.mark.parametrize("rule_id", RULE_IDS)
def test_clean_package_passes_rule(rule_id: str, clean_report: DocumentReport) -> None:
    """Each control produces only PASS findings on the clean closing file."""
    fired = [
...
def test_every_rule_reports_something_on_the_clean_package(
    ...
    assert any(f.rule == rule_id for f in clean_report.findings), (
        f"{rule_id} produced no finding at all on the clean closing file"
    )


def test_clean_package_verdict_is_pass(clean_report: DocumentReport) -> None:
    """The whole closing file rolls up to PASS."""
...
def test_every_rule_declares_a_severity(clean_report: DocumentReport) -> None:
    ...
    :func:`~closing_engine.engine.amount_invalid_finding` and would silently
    default to FAIL.
    """
...
def test_registry_order_is_stable(corpus, clean_report: DocumentReport) -> None:
    """Analyzing the same closing file twice yields findings in the same order."""
