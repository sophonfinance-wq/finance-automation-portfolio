"""
Command-line interface for the home-sale closing & settlement tie-out engine.
=============================================================================
...
::
    ...
    python -m closing_engine <folder> [options]

Analyzes every ``.json`` closing file in ``<folder>`` (read-only) and prints the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="python -m closing_engine",
        description=(
            "Read-only tie-out engine for a residential developer's home-sale closings: every closed unit's balanced journal entry footed, its settlement statement recomputed, its fees struck at the proforma rates, its revenue recognized in the close month and its loan release tied to the lender statement. Analyzes seeded, fictional closing files."
        ),
    )
    p.add_argument("folder", help="folder containing .json closing files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.generate:
        ...
        try:
            ...
            return 3
        if not args.quiet:
            print(f"Generated {len(written)} fictional closing file(s) into {folder}")
    ...
    try:
        ...
        print(f"error: could not read closing files in {folder}: {exc}", file=sys.stderr)
        return 3

    if not reports:
        print(f"error: no .json closing files found in {folder}", file=sys.stderr)
        return 3
    ...
    print(f"\nOverall verdict: {overall.value}  ({len(reports)} closing file(s))")
